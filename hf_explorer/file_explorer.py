...
class FileExplorer(gr.Blocks):
    ...
    def __init__(
        self,
        root_path: str = "/",
        app_path: str = "/code", # Default for Hugging Face Spaces
        glob: str = "*",
        **kwargs
    ):
        """
        ...
        with self:
                ...
                with gr.TabItem("📂 File Explorer"):
                    with gr.Row():
                        ...
                        with gr.Column(scale=1, min_width=250):
                            ...
                            self.path_input = gr.Textbox(
                                label="Enter path and press Enter",
                        ...
                        with gr.Column(scale=3):
                            ...
                            self.file_list_df = gr.DataFrame(
            ...
            # --- Load component and attach event handlers ---
            self.load(
                fn=self.update_file_list,
                inputs=[self.current_dir_state],
                outputs=[self.file_list_df, self.current_path_display, self.current_dir_state]
            )
    ...
    def _attach_event_handlers(self):
        ...
        self.path_input.submit(
            fn=self.update_file_list,
            inputs=[self.path_input],
            outputs=[self.file_list_df, self.current_path_display, self.current_dir_state]
        )

        self.file_list_df.select(
            self.handle_row_select,
            [self.file_list_df, self.current_dir_state],
            [self.file_list_df, self.current_path_display, self.file_content_display, self.selected_file_path, self.current_dir_state]
        )

        self.up_button.click(
            self.go_up,
            [self.current_dir_state],
            [self.file_list_df, self.current_path_display, self.current_dir_state]
        )
        self.home_button.click(
            lambda: self.update_file_list(self.app_path),
            [],
            [self.file_list_df, self.current_path_display, self.current_dir_state]
        )
        self.root_button.click(
            lambda: self.update_file_list(self.root_path),
            [],
            [self.file_list_df, self.current_path_display, self.current_dir_state]
        )
    ...
    def get_file_content(self, filepath):
        try:
            ...
            # Note: The 'file' command is Linux/macOS specific.
            try:
                ...
                # Fallback if 'file' command is not available or fails
                pass
    ...
    def get_disk_usage(self):
        try:
            # Note: 'df' is a Linux/macOS command.
